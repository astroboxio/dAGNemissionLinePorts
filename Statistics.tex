...
In \label{KolmogorovSmirnov} we can see the K-S test performed for the [OII]/[OIII] line ratio. To reject the null hypothesis to a 0.001 level, the K-S statistic, D must be
\begin{equation}
...
D_{n_1,n_2}> 0.26
\end{equation}
The K-S statistic we find has a value of $0.38$, far greater than the above boundary; We can reject the null hypothesis with a p-value of $5.49\times10^{-8}$. This is a clear indication that the two samples came from two different populations, underlining the diverse nature of the two sample. 
